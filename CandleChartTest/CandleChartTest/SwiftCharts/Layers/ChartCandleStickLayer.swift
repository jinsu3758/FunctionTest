...
open class ChartCandleStickLayer<T: ChartPointCandleStick>: ChartPointsLayer<T> {
    ...
    fileprivate let isHiddenRectStroke: Bool
    
    
    public init(xAxis: ChartAxis, yAxis: ChartAxis, chartPoints: [T], itemWidth: CGFloat = 10, strokeWidth: CGFloat = 1, increasingColor: UIColor = UIColor.black, decreasingColor: UIColor = UIColor.white, strokeColor: UIColor? = nil, isHiddenRectStroke: Bool = false) {
        self.itemWidth = itemWidth
        self.strokeWidth = strokeWidth
        self.increasingColor = increasingColor
        self.decreasingColor = decreasingColor
        self.strokeColor = strokeColor
        self.isHiddenRectStroke = isHiddenRectStroke
    ...
    override open func chartContentViewDrawing(context: CGContext, chart: Chart) {
        for screenItem in screenItems {
            ...
            context.setStrokeColor(strokeColor == nil ? screenItem.fillColor.cgColor : strokeColor!.cgColor)
            ...
            if !isHiddenRectStroke { context.stroke(screenItem.rect) }
